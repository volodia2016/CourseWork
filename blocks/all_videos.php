...
<p id="nav_name">Всі відеозаписи</p>
	...
	<div id="video_list">
      ...
      <?php
        ...
        $query = mysql_query("select distinct link, movies.name, author, movies.id from movies
             ...
             where not users.id = '$user_watch_video'");
        ...
        if ($numb_rows >= 1) 
        {
          while ($row = mysql_fetch_array($query))
          {
            ...
            //echo $user_watch_video;
            $query_added_video_ = mysql_query("SELECT id_video FROM users_movies where id_user='$user_watch_video' and id_video='$vid_id'");
            ///while ($row_video = mysql_fetch_array($query_added_video_))
            //$row_video = mysql_fetch_array($query_added_video_);
            //echo $row_video['id_video'];
             //{ echo $row_video['id_video']; echo "<br>";}
            $number_of_rows = mysql_num_rows($query_added_video_);
            //echo $number_of_rows;
           
              //echo "ID_video : ".$row['id'];
            //echo $row_video['id_video'];
            ...
            //print_r($row_video);
            if ($number_of_rows >= 1) {
                echo '<button id="added_video_button">
                  <a href = "#">Додано</a>
                  </button>';
              }
              else
              {
                echo '<button id="add_video_button">
                  <a href = "index.php?sidebar=all_videos&id_video='.$vid_id.'">Додати собі</a>
                  </button>';
              }
            ...
            //echo '<input type="submit" value="Додати собі" name="add_video" id="add_video_button">';
            //echo '</form>';
          }
        }
        else echo "Наразі у вас немає відеозаписів.";
        $vidos_id = $_GET['id_video'];
        if ($_GET['sidebar'] == "all_videos")  
        {
          if ($_GET['id_video']) 
          {
            $query_insert = mysql_query("INSERT INTO users_movies(id_user, id_video) VALUES ('$user_watch_video', '$vidos_id')");
